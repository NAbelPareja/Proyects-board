...
export const Topbar = ({ onMenuClick }) => {
  return (
    <header className="sticky top-0 z-30 flex h-14 items-center justify-between border-b border-slate-200 bg-white px-4 md:px-6 dark:bg-slate-800 dark:text-slate-100">
      <div className="flex items-center gap-3">
        {/* Menú móvil */}
        <button
          onClick={onMenuClick}
          className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 md:hidden dark:bg-slate-800 dark:text-slate-100"
        >
          <FiMenu size={19} className=""/>
        </button>
        ...
        <div className="relative w-52 sm:w-64">
          <FiSearch
            size={14}
            className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 dark:bg-slate-800 dark:text-slate-100"
          />
